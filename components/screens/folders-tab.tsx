    ...
    <div className="overflow-y-auto pb-[76px]">
      ...
      <div className="px-5 pt-14 pb-5">
        <div className="flex items-center justify-between">
          <h2 className="text-[24px] font-bold leading-none tracking-tight text-[#4a4036]">
            My Folders
          </h2>
          <button
      ...
      <section className="px-5 mb-6">
        <p className="mb-3 text-[11px] font-semibold uppercase tracking-wider text-[#b0a396]">
          Default
        </p>
        <button
          onClick={() => setViewing("inbox")}
          className="w-full flex items-center gap-4 rounded-2xl border border-[#e0d8cc] bg-white px-4 py-4 shadow-[0_1px_3px_rgba(74,64,54,0.07)] text-left active:scale-[0.98] transition-transform"
        >
          ...
          <div className="flex-1">
            <p className="text-[14px] font-bold text-[#4a4036]">Inbox</p>
            <p className="text-[12px] text-[#b0a396] mt-0.5">
              {loading ? "Loading…" : `${unsortedCount} unsorted ${unsortedCount === 1 ? "file" : "files"}`}
            </p>
          </div>
          <span className="text-[18px] text-[#b0a396] leading-none">›</span>
        </button>
      </section>
      ...
      <section className="px-5">
        ...
        {loading ? (
          ...
          <EmptyFolders onNew={() => setCreateOpen(true)} />
        ) : (
          <FolderGrid>
...
function EmptyFolders({ onNew }: { onNew: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center gap-3 py-10">
      ...
      <p className="text-[13px] text-[#b0a396]">No folders yet</p>
      <button
        onClick={onNew}
